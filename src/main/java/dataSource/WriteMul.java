...
//            this.name = name;
    ...
    private static String path = "G:\\city.txt";
    ...
    public static void createStream(City[] cities, Order[] orders, String outputFile) throws IOException {
        ...
        try {
            for (int j=0;j<10;j++) {
                int time_phase = last_time *j/10;
                String filename=outputFile+"\\"+j+".txt";
                createFile(filename);
                File file = new File(filename);
                FileOutputStream out = new FileOutputStream(file);
                BufferedOutputStream Buff = new BufferedOutputStream(out);
                System.out.println("时间"+time_phase);
                for (int time = time_phase; time <time_phase+last_time /10; time++) {
                    for (int i = 0; i < len; i++) {
                        if (!orders[i].done) {
...
//                    System.out.println("第"+j+"文件"+time * 10 + "秒各订单定位输出完成");
                }
    ...
    public static void main(String[] args) throws IOException {
        ...
        City[] cities = createCities(1000, "E:\\Desktop\\city.txt");
        System.out.println("生成城市完成");
        Order[] orders = createOrders(cities, 10000, "E:\\Desktop\\orders.txt");
        System.out.println("生成订单完成");
        System.out.println(last_time);
        createStream(cities, orders, "G:\\results");
